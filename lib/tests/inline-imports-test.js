...
fragment textFragment on TextModel {
  text
      ...
      project.files['parent-fragment-import-from-dependency.graphql'] = `
#import 'my-dependency/_dependency-fragment.graphql'
fragment FromDependencyFragment on User {
  ...FromDependency
}`;
...
fragment CustomResolutionFragment on User {
  ...
  it('`inlineImportsWithLineToImports` relays resolveOptions when there are transitive fragments', function () {
    const passedResolveOptions = [];

    function resolveImport(identifier, resolveOptions) {
      passedResolveOptions.push(resolveOptions);

      return resolve(identifier, JSON.parse(JSON.stringify(resolveOptions)));
    }
    ...
    const { inlineImports } = inlineImportsWithLineToImports(
      `
#import './parent-fragment-import-from-dependency.graphql'
query {
  myQuery {
    ...FromDependencyFragment
  }
}`,
      options,
    );
...
query {
  myQuery {
    ...
    expect(passedResolveOptions).to.deep.eql(new Array(11).fill({ basedir, foo: 'bar' }));
  });
});
