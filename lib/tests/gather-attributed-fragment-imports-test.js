...
const fs = require('fs');
const gatherAttributedFragmentImports = require('../gather-attributed-fragment-imports');
...
describe('gather-attributed-fragment-imports', function () {
  let basedir;
...
fragment FromDependency on User {
  ...
  it('handles single import', function () {
    const result = gatherAttributedFragmentImports({
    ...
    assert.deepEqual(
      ...
      new Map([
        [
          2,
          {
            resolvedPath: path.join(basedir, 'apple.graphql'),
    ...
    assert.deepEqual(
      ...
      new Map([
        [
          ...
          {
            resolvedPath: path.join(basedir, 'apple.graphql'),
            fragmentDefinitionsByName: new Map([['apple', { name: { value: 'apple' } }]]),
          },
        ],
        [
          3,
          {
            resolvedPath: path.join(basedir, 'orange.graphql'),
            fragmentDefinitionsByName: new Map([['orange', { name: { value: 'orange' } }]]),
          },
        ],
      ]),
    );
    ...
    assert.deepEqual(
      ...
      new Map([
        [
          2,
          {
            resolvedPath: path.join(basedir, 'nested-1.graphql'),
            fragmentDefinitionsByName: new Map([
      ...
      new Map([
        [
          2,
          {
            ...
            fragmentDefinitionsByName: new Map([
              ['FromDependency', { name: { value: 'FromDependency' } }],
            ]),
          },
        ],
      ]),
  ...
  it('throws when there is missing import', function () {
    const fn = function () {
      gatherAttributedFragmentImports({
