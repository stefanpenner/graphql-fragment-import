...
function* linesWithInlinedImportsOf(
  ...
  for (let line of fileContents.split(EOL_REGEXP)) {
    ...
    if (matched) {
      ...
      try {
        resolvedPath = resolve(importIdentifier, resolveOptions);
      } catch (e) {
      ...
      yield { line, match: true, lineNumber, resolvedPath };
    } else {
...
module.exports.lineToAttributedImports = function (
  fileContents,
  options = {},
  visited = new Set(),
) {
  const result = new Map();
  for (let { line, match, lineNumber, resolvedPath } of linesWithInlinedImportsOf(
    fileContents,
    options,
    visited,
  )) {
    ...
    // non-matching lines
    if (match) {
      result.set(lineNumber, { resolvedPath, line });
    }
  }
  return result;
};
