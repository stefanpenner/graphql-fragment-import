...
if (cli.input.length === 0) {
  ...
  if (entry.isDirectory()) {
    if (!("outputDir" in cli.flags)) {
      throw new Error(
        "When providing a directory of inputs, you must specify --output-dir"
      );
    }
    ...
    for (const queryFile of queryFiles) {
      ...
      console.log(
        chalk`  {green [processed]} ${queryFile} -> ${outputFileName}`
      );
      try {
  ...
  throw new Error(
    `invalid number of inputs, expected '1' but got '${cli.input.length}' `
  );
}
