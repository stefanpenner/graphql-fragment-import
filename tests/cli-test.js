...
describe("cli", function () {
  ...
  describe("some real examples", function () {
    let basedir;
    beforeEach(function () {
      const project = new FixturifyProject(
        "project-name",
        "0.0.0",
        (project) => {
...
query {
  allUsers {
    ...
    it("does not support both output and output-dir at the same time", async function () {
      const childProcess = await execa(
        "node",
        [cli, "-o", "output-file", "-d", "output-dir"],
        {
          reject: false,
        }
      );
    ...
    it("fails if multiple inputs are provide without --output-dir", async function () {
      ...
      expect(childProcess.stdout).to.eql("");
      expect(childProcess.stderr).to.includes(
        `invalid number of inputs, expected '1' but got '2'`
      );
    });
    ...
    it("handles multiple inputs with --output-dir", async function () {
      ...
      expect(childProcess.exitCode).to.eql(0);
      expect(childProcess.stdout).to.eql(
        `  [processed] all-people.graphql -> ${path.join(
          basedir,
          "/output/all-people.graphql"
        )}
  ...
  [processed] some-people.graphql -> ${path.join(
    basedir,
    "/output/some-people.graphql"
  )}`
      );
      expect(childProcess.stderr).to.includes("");
      ...
      expect(
        fs.readFileSync(`${basedir}/output/all-people.graphql`, "UTF8")
      ).to.eql(ALL_PEOPLE_WITH_IMPORTS);
      expect(
        fs.readFileSync(`${basedir}/output/some-people.graphql`, "UTF8")
      ).to.eql(SOME_PEOPLE_WITH_IMPORTS);
      expect(
        fs.readFileSync(`${basedir}/output/nested/other-people.graphql`, "UTF8")
      ).to.eql(NESTED_OTHER_PEOPLE_WITH_IMPORTS);
    });
  });
});
